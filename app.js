...
const btnHeaderAnalytics = document.getElementById("btn-header-analytics");
const analyticsModal = document.getElementById("analytics-modal");
const btnCloseAnalytics = document.getElementById("btn-close-analytics");
const tabRecent = document.getElementById("tab-recent");
const tabBiggest = document.getElementById("tab-biggest");
const panelRecent = document.getElementById("analytics-recent-panel");
const panelBiggest = document.getElementById("analytics-biggest-panel");
const recentList = document.getElementById("recent-changes-list");
const biggestList = document.getElementById("biggest-changes-list");
...
function setupEventListeners() {
    ...
    btnHeaderAnalytics.addEventListener("click", openAnalytics);
    btnCloseAnalytics.addEventListener("click", closeAnalytics);
    tabRecent.addEventListener("click", () => switchAnalyticsTab('recent'));
    tabBiggest.addEventListener("click", () => switchAnalyticsTab('biggest'));
    analyticsModal.addEventListener("click", (e) => {
        if (e.target === analyticsModal || e.target.classList.contains("modal-handle")) {
            closeAnalytics();
        }
    });
...
// Open analytics modal sheet and pause camera
function openAnalytics() {
    if (isScanning) {
        stopScanner();
    }
    analyticsModal.classList.add("active");
    // Default to recent updates tab on open
    switchAnalyticsTab('recent');
}

// Close analytics modal sheet and resume scanning
function closeAnalytics() {
    analyticsModal.classList.remove("active");
    setTimeout(() => {
        if (!isScanning && !priceModal.classList.contains("active") && !basketModal.classList.contains("active")) {
            startScanner();
        }
    }, 300);
}
...
function switchAnalyticsTab(tab) {
    if (tab === 'recent') {
        ...
        panelRecent.classList.remove('hidden');
        panelBiggest.classList.add('hidden');
        fetchRecentChanges();
    } else {
        tabRecent.classList.remove('active');
        tabBiggest.classList.add('active');
        panelRecent.classList.add('hidden');
        panelBiggest.classList.remove('hidden');
        fetchBiggestChanges();
    }
}

// Format time string to "X time ago"
...
// Fetch recently changed prices from Supabase
async function fetchRecentChanges() {
    recentList.innerHTML = `<div class="list-empty-msg">Loading recent changes...</div>`;
    
    const hasCloud = config.supabaseUrl && config.supabaseKey;
    if (!hasCloud) {
        recentList.innerHTML = `<div class="list-empty-msg">Supabase is not configured.</div>`;
        return;
    }
    
    try {
        const url = `${config.supabaseUrl}/rest/v1/price_history?select=*&order=changed_at.desc&limit=20`;
        const response = await fetch(url, {
            method: "GET",
            headers: {
                "apikey": config.supabaseKey,
        ...
        if (response.ok) {
            const data = await response.json();
            renderChangesList(recentList, data);
        } else {
            recentList.innerHTML = `<div class="list-empty-msg">Failed to load changes (Status ${response.status})</div>`;
        }
    } catch (err) {
        console.error("Error fetching recent changes", err);
        recentList.innerHTML = `<div class="list-empty-msg">Connection error. Could not fetch logs.</div>`;
    }
}

// Fetch biggest price changes from Supabase
async function fetchBiggestChanges() {
    biggestList.innerHTML = `<div class="list-empty-msg">Loading biggest changes...</div>`;
    
    const hasCloud = config.supabaseUrl && config.supabaseKey;
    if (!hasCloud) {
        biggestList.innerHTML = `<div class="list-empty-msg">Supabase is not configured.</div>`;
        return;
    }
    
    try {
        const url = `${config.supabaseUrl}/rest/v1/price_history?select=*&order=change_amount.desc&limit=20`;
        const response = await fetch(url, {
            method: "GET",
            headers: {
                "apikey": config.supabaseKey,
        ...
        if (response.ok) {
            const data = await response.json();
            renderChangesList(biggestList, data);
        } else {
            biggestList.innerHTML = `<div class="list-empty-msg">Failed to load changes (Status ${response.status})</div>`;
        }
    } catch (err) {
        console.error("Error fetching biggest changes", err);
        biggestList.innerHTML = `<div class="list-empty-msg">Connection error. Could not fetch logs.</div>`;
    }
}

// Helper to render changes log list
function renderChangesList(containerElement, data) {
    if (!data || data.length === 0) {
        containerElement.innerHTML = `<div class="list-empty-msg">No price updates recorded yet. Try updating a price in your POS SQLite database to trigger a log!</div>`;
        return;
    }
    ...
    data.forEach(item => {
        const isUp = item.new_price > item.old_price;
        const diffAmount = item.change_amount;
        const relativeTime = formatRelativeTime(item.changed_at);
        ...
        const itemHtml = `
            <div class="change-item">
                <div class="change-info">
                    ...
                    <div class="change-barcode-time">
                        <span>Barcode: ${item.barcode}</span>
                        <span>•</span>
                        <span class="change-time">${relativeTime}</span>
                    </div>
                </div>
                <div class="change-pricing">
                    <span class="change-amount-badge ${isUp ? 'price-up' : 'price-down'}">
                        ${isUp ? '▲' : '▼'} ${diffAmount.toLocaleString()} IQD
                    </span>
                    <div class="change-flow">
                        <span class="old-price">${item.old_price.toLocaleString()}</span>
                        <span> → </span>
                        <span class="new-price">${item.new_price.toLocaleString()} IQD</span>
                    </div>
                </div>
            </div>
        `;
